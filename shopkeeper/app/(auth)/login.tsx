...
export default function ShopkeeperLoginScreen() {
  ...
  const handleDemoSignIn = async () => {
    setLoading(true);
    const res = await authService.signInShopkeeper('srisai.kirana@example.com', 'demo123');
    setLoading(false);
    if (res.success) {
      if (res.shop?.id) {
        useShopStore.getState().setActiveShopkeeperShopId(res.shop.id);
      }
      router.replace('/(tabs)/dashboard' as any);
    } else {
      showAlert('Demo Login', 'Welcome to Store Owner Portal!');
      router.replace('/(tabs)/dashboard' as any);
    }
  };
  ...
  const handleAuthSubmit = async () => {
    ...
    if (mode === 'signup') {
      ...
      if (!shopName.trim()) {
        showAlert('Missing Shop Name', 'Please enter your Store Name.');
        return;
      }
      if (!phone.trim()) {
        showAlert('Missing Phone', 'Please enter your 10-digit phone number.');
        return;
      }
      ...
      const res = await authService.signUpShopkeeper({
        ...
        address: address.trim() || 'Road No. 12, Banjara Hills, Hyderabad',
      });
      setLoading(false);
      ...
      if (res.success) {
        ...
        showAlert('Registration Error', res.error || 'Failed to register account.');
      }
    } else {
      // Sign In Flow
      setLoading(true);
      ...
      if (res.success) {
        ...
        showAlert('Login Error', res.error || 'Invalid email or password. Please check your credentials.');
      }
    }
  };
            ...
            {mode === 'signin' && (
              ...
              >
                ...
                <Text style={{ fontSize: 13, fontWeight: '700', color: '#059669' }}>
                  ⚡ Quick Demo Store Sign In (Sri Sai Kirana)
                </Text>
              </TouchableOpacity>
            )}
