...
export default function AddProductScreen() {
  ...
  const handleSave = () => {
    if (!name.trim()) {
      showAlert('Missing Name', 'Please enter a product name (e.g. Fresh Milk, Basmati Rice).');
      return;
    }
    ...
    const targetShopId = activeShop?.id || (shops.length > 0 ? shops[0].id : 'shop_demo_1');
    ...
    setAddedItemDetails({
      name: name.trim(),
      price: priceNum,
      shop: activeShop?.name || 'Sri Sai Kirana & General Store',
    });
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      <View style={styles.topTitleBar}>
        ...
        <Text style={styles.topTitleBarText}>Add New Product</Text>
        <View style={{ width: 36 }} />
      </View>
